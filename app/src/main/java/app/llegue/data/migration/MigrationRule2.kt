...
package app.llegue.data.migration

import io.realm.DynamicRealm

class MigrationRule2 : MigrationRule {

    override fun migrate(realm: DynamicRealm) {
        // This is beta, so we can just recreate schema
        val schema = realm.schema
        val sms = schema.get("Sms")!!
        sms.addRealmObjectField("destinationContact", schema.get("Contact")!!)
    }

    override fun toVersion(): Long = 2
}